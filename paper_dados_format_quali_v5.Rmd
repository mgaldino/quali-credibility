...
P1: Introducing the Puzzle and Main Literature Explanation
A pesquisa qualitativa em ciência política, especialmente no estudo da causalidade, enfrenta um desafio metodológico ainda pouco compreendido: sua suposta limitação na capacidade de generalização. Métodos como a análise comparativa de casos [@Rihoux_Ragin_2009; @Ragin_2014] e process tracing [@Collier_2011; @Bennett_Checkel_2015] são frequentemente considerados menos robustos que abordagens quantitativas precisamente porque suas inferências seriam restritas a contextos específicos e limitados por amostras pequenas. Essa concepção encontra respaldo em trabalhos clássicos, como o de @King_etal_1994 (KKV), que propôs a unificação da lógica da pesquisa em ciências sociais sob um paradigma quantitativo, que supostamente teria uma capacidade de generalização estabelecida. 

P2: Acknowledging Theoretical Ambiguities and Differentiating Motivations
Entretanto, a chamada revolução da credibilidade, ao enfatizar a importância de suposições críveis e transparentes para assegurar que os efeitos causais possuam validade interna, deixou claro que nada podemos dizer, a princípio, sobre a generalização para além da população alvo de um estudo – validade externa. Além disso, a revolução da inferência Bayesiana na estatística também traz questionamentos sobre a necessidade de amostras grandes para fazer inferência estatística. Portanto,  é preciso investigar de maneira mais precisa em que medida existem diferenças intrínsecas entre métodos qualitativos e quantitativos no que diz respeito a sua capacidade de generalização.

P3: Critique of Previous Work and Research Gap
Apesar das críticas levantadas contra a visão proposta por KKV, como aquelas discutidas em Rethinking Social Inquiry [@brady_collier_2010], grande parte das respostas dentro da metodologia qualitativa manteve a suposição de que de fato a pesquisa qualitativa é limitada em termos de generalização [@Amorim_Rodriguez_2016, @blatter_haverland_2012; @brady_collier_2010]. O foco dos contra argumentos passou por criticar o custo de aumentar o tamanho da amostra e a consequente perda de contexto, em defender a necessidade de diferenciar  data-observations e  observações de processos causais em “within case analysis” e em objetivos diferentes da pesquisa qualitativa [@brady_etal_2010]. Em última instância, no entanto, concedem capacidade de generalização para estudos quantitativos e a suposta necessidade de aumentar o tamanho da amostra para inferência.
No entanto, a literatura recente sobre inferência causal sugere que essa premissa pode ser equivocada. Se a credibilidade das suposições for o fator determinante para a validade de uma inferência causal, então as críticas dirigidas aos métodos qualitativos devem ser reformuladas. Além disso, a incorporação de ferramentas da inferência Bayesiana aos métodos qualitativos pode oferecer soluções para desafios de $n$ pequeno enfrentados por esses métodos. Esse espaço teórico ainda não foi totalmente explorado na literatura metodológica, o que cria uma lacuna importante na compreensão da validade interna e externa dos métodos qualitativos sob a perspectiva da Revolução da Credibilidade e da estatística Bayesiana.

P4: Introducing the Current Study
Esta nota de pesquisa propõe uma nova abordagem para avaliar a pesquisa qualitativa causal à luz dos desenvolvimentos da Revolução da Credibilidade e da inferência Bayesiana. Argumentamos que a distinção entre métodos qualitativos e quantitativos precisa ser reformulada para refletir melhor a relação entre identificação causal e inferência estatística. Utilizando os avanços metodológicos recentes, mostramos que a pesquisa qualitativa pode alcançar níveis de rigor inferencial comparáveis aos dos métodos quantitativos, desde que as suposições de identificação causal sejam explicitadas e avaliadas corretamente.
P5. Outlining the Paper's Contributions
O estudo contribui para a literatura metodológica de quatro maneiras principais. Primeiro, esclarece a distinção entre identificação causal e inferência estatística, demonstrando que a validade interna não é exclusiva dos métodos quantitativos. Segundo, contribui para uma literatura recente [Fairfield_Charman_2022; @Humphreys_Jacobs_2023] que tem explorado como a estatística Bayesiana pode ser integrada ao process tracing e outras abordagens qualitativas para fortalecer a inferência causal em estudos de pequeno-n. Terceiro, propõe uma reformulação das críticas metodológicas dirigidas à pesquisa qualitativa, sugerindo que a questão central não é a generalização, mas sim a credibilidade das suposições que sustentam a identificação causal. Por fim, contribui para auxiliar no ensino e pesquisa de métodos qualitativos, de forma que suas forças e limitações possam ser mais bem apreciadas e não sejam desvalorizadas na pesquisa e ensino de ciência política.

P6. Linking Findings to Broader Contexts and Implications
As implicações deste estudo vão além do debate metodológico em ciência política. Ao demonstrar que a Revolução da Credibilidade e a inferência Bayesiana podem fortalecer os métodos qualitativos, este trabalho contribui para uma agenda de pesquisa mais integrada, onde métodos qualitativos e quantitativos não são vistos como concorrentes, mas sim como abordagens complementares. A adoção dessas ferramentas pode transformar a forma como conduzimos pesquisa empírica em ciência política, permitindo inferências mais robustas e confiáveis, independentemente do tamanho da amostra. Ao desafiar concepções tradicionais sobre a validade dos métodos qualitativos, esse estudo busca estimular um debate mais amplo sobre as melhores práticas metodológicas nas ciências sociais.
...
Potanto, de saída podemos descartar como infundadas duas posições. Se, como a literatura metodológica argumenta, pesquisa qualitativa não tem maiores problemas e atingir validade interna, então não há qualquer diferença com relação a esse aspecto em métdos qualitativos ou quantitativos.

Similarmente, as duas metodologias são igualmente limitadas (a princípio) em sua capacidade de possuírem validade externa. E essas duas conclusões seguem do fato de que não há relação entre validade interna e externa e tamanho amostral, se a pesquisa é em profundidade ou não, ou quaisquer das possíveis distinções empregadas para diferenciar método qualitativo de quantitativo.
...
Esta seção introduz um exemplo hipotético simplicado para demostrar como a inferência Bayesiana e o *process tracing* podem ser aplicados e permiter inferências mesmo com amostra reduzida e sem depender de uma priori informativa. O exemplo mostra como a pesquisa qualitativa depende também de um processo de geração de dados (PGD) de discrimine as hipóteses rivais em termos de implicações observáveis. Essa é a chave para inferências quantitativas em estudos de caso.

#### Teste de hipótese com PGD discriminativo

Suponha que um mágico profissional afirma ter a capacidade de jogar uma moeda de tal modo que ela sempre dá "cara" (Hipótese 0, ou $h0$). Após examinar a moeda e perceber que ela possui os dois lados, cara e coroa, você pede por uma evidência empírica. Sua hipótese rival é que a chance de sair "cara" é 50%. O mágio então lança a moeda quatro vezes, as quais resultam todas em "cara". 
Consider a magician who claims the ability to flip a coin such that it always lands on heads. The rival hypothesis is that the coin is fair, meaning it has an equal probability (50%) of landing heads or tails. To evaluate these competing hypotheses, we observe four coin flips, all of which result in heads. We are interested in calculating the likelihood of observing this data under each hypothesis.

Nós sabemos que, pelo teorema de Bayes, a posteriori é dada por: $Prob(H_0|\text{4 caras}) \propto \text{priori de }H_0 \times \text{verossimilhança} = Prob(H_0) \times Prob(\text{4 caras}|H_0)$, de modo que a razão de posterioris $\frac{Prob(H_0|\text{4 caras})}{Prob(H_1|\text{4 caras})} = \frac{Prob(H_0) \times Prob(\text{4 caras}|H_0)}{\Prob(H_1) \times Prob(\text{4 caras}|H_1)}$. Supondo iqual probabilidade a priori para cada hipótese (portanto uma priori não-informativa), temos a razão das posterioris é apenas a razão das verossimilhanças, que nesse caso é $\frac{1ˆ4}{.5^4} = 16$. Ou seja, dizemos que a evidência é tal que a hipótese do mágico conseguir controlar o resultado do lançamento da moeda é $16$ vezes mais provável do quu a hipótese alternativa de que ele não possui nenhum controle. Isso com apenas quatro observações.

Uma pesquisadora pode considerar que é mais provável que o mágico possua algum controle, ainda que não perfeito. Por exemplo, que consegue obter cara 75% das vezes ($H_2$). Se essa for a hipótese rival, então podemos novamente calcular a razão de posterioris, que nesse caso dá aproximadamente $3.16$ vezes mais provável a hipótese $H_0$ do que $H_2$.

Na pesquisa qualitativa, por exemplo em *process tracing*, é comum termos mais de uma observação para o mesmo caso, em geral relativo ao mecanismo. Por exemplo, se $H_0$ for verdadeira, devemos esperar que a altura que a moeda alcança em cada lançamento seja a mesma, enquanto no caso das hipóteses rivais, a altura deve variar de um lançamento para outro. Esse é um mecanismo causal suposto por cada hipótese. Digamos que observamos alturas iguais nos quatro lançamentos. 


Hypothesis 1 (Magician's Claim): The coin always lands on heads, so the probability of observing heads is 
P
(
H
)
...
1
P(H)=1.
Hypothesis 2 (Fair Coin): The probability of heads or tails is 
P
(
H
)
